...
// Основные фичи: прогресс-бар, вкладки, лог (спойлер), сохранение ini, опции yt-dlp
// Компиляция: CMake + MSVC. Требует comctl32 v6 (манифест включён).
...
std::wstring DecodeBytes(const std::vector<BYTE>& buf, int forcedCp /*0=auto, 65001, 1251, 866*/) {
    ...
    if (forcedCp == 65001) {
        ...
        return w;
    } else if (forcedCp == 1251 || forcedCp == 866) {
        ...
        // Auto: prefer UTF-8 if valid, else CP1251, else CP866
        if (IsValidUTF8(buf)) {
            int len = MultiByteToWideChar(CP_UTF8, 0, (LPCSTR)buf.data(), (int)buf.size(), NULL, 0);
            std::wstring w(len, L'\0');
            MultiByteToWideChar(CP_UTF8, 0, (LPCSTR)buf.data(), (int)buf.size(), w.data(), len);
...
// -------------------- Layout --------------------
RECT g_Client{};
int g_TopControlsHeight = 120;
int g_ProgressHeight = 26;
int g_LogHeight = 180;
int g_MinWidth = 800;
int g_MinHeight = 600;
...
void DoLayout() {
    ...
    MoveWindow(CreateWindowW(L"STATIC", L"URL:", WS_CHILD|WS_VISIBLE, x, y+4, 50, 20, g_hWnd, 0, g_hInst, 0), 0,0,0,0, FALSE); // dummy
    ...
    MoveWindow(CreateWindowW(L"STATIC", L"Папка:", WS_CHILD|WS_VISIBLE, x, y+4, 110, 20, g_hWnd, 0, g_hInst, 0), 0,0,0,0, FALSE);
...
HWND CreateLabeledEdit(HWND parent, int idEdit, const wchar_t* label, int x, int y, int w, int h, int labelWidth=120) {
    CreateWindowW(L"STATIC", label, WS_CHILD|WS_VISIBLE, x, y+4, labelWidth, 20, parent, 0, g_hInst, 0);
    return CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"", WS_CHILD|WS_VISIBLE|ES_AUTOHSCROLL, x+labelWidth+4, y, w-labelWidth-4, h, parent, (HMENU)idEdit, g_hInst, 0);
}
HWND CreateCheck(HWND parent, int id, const wchar_t* text, int x, int y) {
    return CreateWindowW(L"BUTTON", text, WS_CHILD|WS_VISIBLE|BS_AUTOCHECKBOX, x, y, 300, 22, parent, (HMENU)id, g_hInst, 0);
}
HWND CreateCombo(HWND parent, int id, int x, int y, int w, int h) {
    return CreateWindowW(L"COMBOBOX", L"", WS_CHILD|WS_VISIBLE|CBS_DROPDOWNLIST|WS_VSCROLL, x, y, w, h, parent, (HMENU)id, g_hInst, 0);
}
HWND CreateButton(HWND parent, int id, const wchar_t* text, int x, int y, int w, int h) {
    return CreateWindowW(L"BUTTON", text, WS_CHILD|WS_VISIBLE|BS_PUSHBUTTON, x, y, w, h, parent, (HMENU)id, g_hInst, 0);
}
HWND CreateMultilineEdit(HWND parent, int id, int x, int y, int w, int h) {
    return CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"", WS_CHILD|WS_VISIBLE|ES_MULTILINE|ES_AUTOVSCROLL|ES_AUTOHSCROLL|WS_VSCROLL, x, y, w, h, parent, (HMENU)id, g_hInst, 0);
}

void CreateTopControls(HWND hWnd) {
    // URL
    hEditUrl = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"", WS_CHILD|WS_VISIBLE|ES_AUTOHSCROLL, 8+60, 8, 600, 24, hWnd, (HMENU)ID_EDIT_URL, g_hInst, 0);
    CreateWindowW(L"STATIC", L"URL:", WS_CHILD|WS_VISIBLE, 8, 12, 50, 20, hWnd, 0, g_hInst, 0);
    ...
    // Save folder
    hEditSaveFolder = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"", WS_CHILD|WS_VISIBLE|ES_AUTOHSCROLL, 8+120, 8+32, 600, 24, hWnd, (HMENU)ID_EDIT_SAVEFOLDER, g_hInst, 0);
    CreateWindowW(L"STATIC", L"Папка:", WS_CHILD|WS_VISIBLE, 8, 8+32+4, 110, 20, hWnd, 0, g_hInst, 0);
    hBtnBrowseSaveFolder = CreateButton(hWnd, ID_BTN_BROWSE_SAVEFOLDER, L"Обзор...", 0,0,0,0);

    // Progress
    hProgress = CreateWindowExW(0, PROGRESS_CLASSW, NULL, WS_CHILD|WS_VISIBLE, 8, 8+64, 400, 20, hWnd, (HMENU)ID_PROGRESS, g_hInst, 0);
    SendMessageW(hProgress, PBM_SETRANGE, 0, MAKELPARAM(0, 100));
    hLblProgress = CreateWindowW(L"STATIC", L"0%", WS_CHILD|WS_VISIBLE|SS_RIGHT, 0,0,60,20, hWnd, (HMENU)ID_LBL_PROGRESS, g_hInst, 0);
    ...
    hTabs = CreateWindowExW(0, WC_TABCONTROLW, L"", WS_CHILD|WS_VISIBLE|WS_CLIPSIBLINGS|WS_CLIPCHILDREN,
                            8, 8+64+28+28, 600, 300, hWnd, (HMENU)ID_TABS, g_hInst, 0);
    ...
    // Pages
    hPageMain = CreateWindowExW(0, L"STATIC", L"", WS_CHILD|WS_VISIBLE, 0,0,0,0, hTabs, 0, g_hInst, 0);
    hPageNet = CreateWindowExW(0, L"STATIC", L"", WS_CHILD, 0,0,0,0, hTabs, 0, g_hInst, 0);
    hPageFiles = CreateWindowExW(0, L"STATIC", L"", WS_CHILD, 0,0,0,0, hTabs, 0, g_hInst, 0);
    hPageEmbed = CreateWindowExW(0, L"STATIC", L"", WS_CHILD, 0,0,0,0, hTabs, 0, g_hInst, 0);
    hPageChapters = CreateWindowExW(0, L"STATIC", L"", WS_CHILD, 0,0,0,0, hTabs, 0, g_hInst, 0);
    hPageAdv = CreateWindowExW(0, L"STATIC", L"", WS_CHILD, 0,0,0,0, hTabs, 0, g_hInst, 0);
    ...
    // Page: Основное
    {
        ...
        CreateWindowW(L"STATIC", L"--merge-output-format:", WS_CHILD|WS_VISIBLE, 8, 98, 160, 22, hPageMain, 0, g_hInst, 0);
    ...
    // Page: Сеть/Загрузчик
    {
        ...
        CreateWindowW(L"STATIC", L"--downloader:", WS_CHILD|WS_VISIBLE, 8, 40, 120, 22, hPageNet, 0, g_hInst, 0);
        ...
        CreateWindowW(L"STATIC", L"Сопоставления:", WS_CHILD|WS_VISIBLE, 8, 100, 120, 22, hPageNet, 0, g_hInst, 0);
        HWND listMap = CreateWindowExW(WS_EX_CLIENTEDGE, L"LISTBOX", L"", WS_CHILD|WS_VISIBLE|LBS_NOTIFY|WS_VSCROLL, 8, 120, 400, 100, hPageNet, (HMENU)ID_LIST_DL_MAP, g_hInst, 0);
    ...
    // Page: Файлы/Куки/Кэш
    {
        ...
        CreateLabeledEdit(hPageMain, ID_EDIT_CONFIG_LOCATIONS, L"--config-locations:", 8, 130, 600, 24); // по смыслу к "Основное"
    }
    ...
    // Page: Эмбед/Субтитры
    {
        ...
        CreateWindowW(L"STATIC", L"--convert-subs:", WS_CHILD|WS_VISIBLE, 240, 10, 120, 22, hPageEmbed, 0, g_hInst, 0);
        HWND c1 = CreateCombo(hPageEmbed, ID_COMBO_CONVERT_SUBS, 360, 8, 160, 200);
        AddComboItems(c1, {L"none", L"ass", L"lrc", L"srt", L"vtt"}, 0);

        CreateWindowW(L"STATIC", L"--convert-thumbnails:", WS_CHILD|WS_VISIBLE, 240, 42, 140, 22, hPageEmbed, 0, g_hInst, 0);
    ...
    // Page: Дополнительно
    {
        CreateWindowW(L"STATIC", L"Кодировка лога:", WS_CHILD|WS_VISIBLE, 8, 8, 120, 22, hPageAdv, 0, g_hInst, 0);
...
DWORD WINAPI ReaderThread(LPVOID lpParam) {
    RunParams* rp = (RunParams*)lpParam;
    ...
    SetEnvironmentVariableW(L"PYTHONLEGACYWINDOWSSTDIO", L"1"); // иногда помогает
    ...
    // CreateProcessW требует мутабельную строку
    std::wstring cl = app + L" " + rp->cmdline;
    wchar_t* cmd = _wcsdup(cl.c_str());
    if (!CreateProcessW(NULL, cmd, NULL, NULL, TRUE, CREATE_NO_WINDOW, NULL, NULL, &si, &pi)) {
        std::wstring msg = L"[ERR] Не удалось запустить yt-dlp. Проверьте, что yt-dlp.exe доступен. WinErr=" + std::to_wstring(GetLastError()) + L"\r\n";
        PostLog(msg);
        free(cmd);
        goto cleanup;
    }
    ...
    while (true) {
        ...
        if (!ok || rd == 0) break;
        // сохраняем сырые байты (для JSON)
        if (rp->jsonMode) {
...
std::wstring BuildCmdlineCommon(bool listFormats, bool jsonMode) {
    ...
    for (int i=0; i<mapCount; ++i) {
        ...
        // ожидаем формат "proto_list:NAME" уже собранный при добавлении
        args.push_back(L"--downloader");
...
void LoadSettings() {
    ...
    int encIdx = std::stoi(IniRead(L"log_encoding_idx", L"0"));
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_COMMAND: {
        ...
        if (id == ID_BTN_BROWSE_SAVEFOLDER) {
            ...
            DoLayout();
        } else if (id == ID_TABS && code == TCN_SELCHANGE) {
            ...
            // создаём строку "proto_list:NAME"
